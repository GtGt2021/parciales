...
static int utn_obtenerLetrasyNumeros(char cadena[], int len);
...
int utn_esLetraYNumero(char cadena[])
{
	int retorno=-1;
	int i=0;
	if(cadena != NULL && strlen(cadena)>0)
	{
		...
		while (cadena[i]!='\0')
		{
			if( (cadena[i]<32) || (cadena[i]>32&&cadena[i]<48) || (cadena[i]>57 && cadena[i]<65)|| (cadena[i]>90 && cadena[i]<97) || cadena[i]>122)
			{
				retorno=-1;
				break;
			}
...
static int utn_obtenerLetrasyNumeros(char cadena[], int len)
{
	...
	if (cadena!=NULL && len>0)
	{
		...
		if (utn_miFgets(buffer, sizeof(buffer))==0 && utn_esLetraYNumero(buffer)==0)
		{
			strcpy(cadena, buffer);
			retorno=0;
		}
...
int utn_pedirLetrasyNumerosConEspacioAUsuario(char cadena[], int len, int reintentos, char* variableTexto, char* textoError)
{
	int retorno=-1;
	if(cadena != NULL && reintentos >=0 && variableTexto != NULL && textoError != NULL && len>0)
	{
		do
		{
			printf("%s\n",variableTexto);
			fflush( stdin );
			if (utn_obtenerLetrasyNumeros(cadena, len)==0)
			{
				retorno = 0;
				break;
			}
