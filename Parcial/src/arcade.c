...
{
	eArcades* pArcade=(eArcades*)malloc(sizeof(eArcades));
	return pArcade;
}

eArcades* juegoNuevo(void)
{
	eArcades* pArcade=(eArcades*)malloc(sizeof(eArcades));
	return pArcade;
}
...
int juegos_initArray(eJuegos* list[], int len)
{
	...
	if(list!=NULL && len>0)
	{
		for(i=0; i<len; i++)
		{
			list[i]=NULL; // inicializamos en null
		}
		retorno=0;
	}
	return retorno;
...
int arcade_cargarArcade(eArcades* arcade, int idSalon)
{
	...
	if (arcade!=NULL && idSalon>0)
	{
		...
		{
			idArcade=getId();
			arcade_addSalon(arcade, nombreJuego, nacionalidad, idArcade, tipoSonido, idSalon, cantidadJugadores, capacidadFichas);
			retorno=0;
		}
		else
		{
...
int arcade_buscarPosicionLibre(eArcades* arcade[], int len, int *index)
{
	...
	int retorno=-2;
	if (arcade!=NULL && len>0 && index!=NULL)
	{
...
{
	int retorno=-1;
	if (list!=NULL && len>0 && idBuscado>0 && posicionIdBuscado!=NULL)
	{
		for(int i=0; i<len; i++)
		{
			if(list[i]!=NULL && list[i]->id_arcade==idBuscado)
			{
				*posicionIdBuscado=i;
				retorno=0;
				break;
...
int arcade_bajarArcadeById(eArcades* list[], int len, int idBuscado)
{
	...
	if (list!=NULL && len>0 && idBuscado>0)
	{
		for(int i=0; i<len; i++)
		{
			if(list[i]!=NULL && list[i]->idSalon_arcade==idBuscado)
			{
				free(list[i]);
				list[i]=NULL;
				retorno=0;
...
{
	int retorno=-1;
	juegos_initArray(juegos, lenJuegos);
	int indexJuegos=0;
